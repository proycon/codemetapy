...
def parse_classifier(
    value: str, g: Graph, res: Union[URIRef, BNode], crosswalk, args: AttribDict
):
    ...
    if pipkey in crosswalk[CWKey.PYPI]:
        ...
        add_triple(
            g,
            res,
            "runtimePlatform" if key == "programmingLanguage" else key,
            value,
            args,
        )
        ...
        print(
            "NOTICE: Classifier " + fields[0] + " has no translation", file=sys.stderr
        )
...
def parse_url(
    ...
    label = label.lower()
    if "repository" in label or label in (
        "git",
        "github",
        "code",
        "sourcecode",
...
def parse_python(
    g: Graph, res: Union[URIRef, BNode], packagename: str, crosswalk, args: AttribDict
):
    ...
    * `packagename` - Either 1) a name of a package that is installed or available in the current working directory, or 2) path to a pyproject.toml file
    ...
    if os.path.basename(packagename) == "pyproject.toml":
        print(
            f"Loading metadata from {packagename} via pyproject-parser", file=sys.stderr
        ...
        try:
            ...
            if not metadata:
                print(
                    f"Failed to find complete enough metadata in pyproject.toml",
                    file=sys.stderr,
                )
        except Exception as e:
            print(
                f"Failed to process {packagename} via pyproject-parser: {str(e)}",
                file=sys.stderr,
            )

        if not metadata:
            print(
                f"Fallback: Loading metadata from {packagename} via PEP517",
                file=sys.stderr,
            )  # pylint: disable=W0212
            try:
                ...
                print(
                    f"Failed to process {packagename} via PEP517: {str(e)}",
                    file=sys.stderr,
                )
                sys.exit(4)
        ...
        print(
            f"Loading metadata from {packagename} via importlib.metadata",
            file=sys.stderr,
        )  # pylint: disable=W0212
        try:
    ...
    for key, value in metadata:
        if key == "Classifier":  # importlib.metadata
            ...
            if key == "authors" and isinstance(value, (list, tuple)):  # pyproject
            ...
            elif key in ("gui_scripts", "gui-scripts") and isinstance(
                value, dict
            ):  # pyproject.toml, flit, poetry?
                for script_name, rawentrypoint in value.items():
                    ...
                    add_entrypoint(
                        g, res, script_name, module_name, interfacetype, args
                    )
                ...
                print(
                    "WARNING: No translation for distutils or pyproject.toml key "
                    + key,
                    file=sys.stderr,
                )
    ...
    if args.with_stypes and pkg:
        ...
        for rawentrypoint in pkg.entry_points:
            ...
            if rawentrypoint.value:
                ...
                add_entrypoint(
                    g, res, rawentrypoint.name, module_name, interfacetype, args
                )
            else:
...
def add_dependency(g: Graph, res: Union[URIRef, BNode], value: str, args: AttribDict):
    for dependency in splitdependencies(value):
        ...
        if dependency and not dependency.startswith(("<", ">", "=", "^")):
            ...
            depres = URIRef(
                generate_uri(
                    dependency + depversion.replace(" ", ""), args.baseuri, "dependency"
                )
...
def add_entrypoint(
    ...
    name: str,
    module_name: str,
    interfacetype: Union[BNode, URIRef],
    args: AttribDict,
):
    ...
    targetproduct = URIRef(
        generate_uri(
            name,
            baseuri=args.baseuri,
            prefix=get_last_component(str(interfacetype)).lower(),
        )
        + "/"
        + str(version)
    )
...
def test_and_set_library(
    ...
    for _, _, cat in g.triples((res, SDO.applicationCategory, None)):
        ...
        isweb = (
            isweb
            or str(cat).lower().find("internet") != -1
            or str(cat).lower().find("web") != -1
            or str(cat).lower().find("www") != -1
        )
    ...
    if (not found_entrypoints and not isweb) or islibrary:
        ...
        )  # see https://python.github.io/peps/pep-0503/
        if args.exactplatformversion:
            g.add(
                (
                    targetproduct,
